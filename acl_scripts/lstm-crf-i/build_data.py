...
def pre_main():
    ...
    PIO = ['Nolabel', 'surgical', 'aphysical', 'bpharmacological', 'educational', 'cpsychological', 'other', 'control']
    #for pio in PIO:
    print('Reading files for Interventions')
    crowd_labels = glob('../../ebm_nlp_1_00/annotations/aggregated/hierarchical_labels/interventions/train/*.ann')
    for fname in crowd_labels: crowd_ids.add(fname_to_pmid(fname))

    test_labels = glob('../../ebm_nlp_1_00/annotations/aggregated/hierarchical_labels/interventions/test/gold/*.ann')
    ...
    for ids, fname in [(dev_ids, 'dev'), (test_ids, 'test')]:
      fout = open('data/%s.txt' %fname, 'w')
      for pmid in ids:
        fout.write('-DOCSTART- -X- O O\n\n')
        for i, (token, pos) in enumerate(zip(id_to_tokens[pmid], id_to_pos[pmid])):
